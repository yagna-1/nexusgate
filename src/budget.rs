...
impl BudgetEnforcer {
    ...
    fn daily_key(key_id: &str) -> String {
        let today = Utc::now().format("%Y-%m-%d");
        format!("{}{key_id}:daily:{today}", BUDGET_PREFIX)
    }

    fn monthly_key(key_id: &str) -> String {
        let ym = Utc::now().format("%Y-%m");
        format!("{}{key_id}:monthly:{ym}", BUDGET_PREFIX)
    }
    ...
    pub async fn check(&self, ctx: &AuthContext, estimated_micro_usd: i64) -> Result<()> {
        ...
        if let Some(daily_limit) = ctx.budget_daily_micro_usd {
            let spent = self.get_daily_spend(&ctx.key_id).await;
        ...
        if let Some(monthly_limit) = ctx.budget_monthly_micro_usd {
            let spent = self.get_monthly_spend(&ctx.key_id).await;
    ...
    pub async fn record_cost(
        ...
        // Update Redis counters atomically (fast, for budget checking)
        {
            ...
            let daily_key = Self::daily_key(key_id);
            let monthly_key = Self::monthly_key(key_id);
            ...
            // 32 days TTL
            let _: () = conn.expire(&monthly_key, 32 * 24 * 3600).await.unwrap_or(());
        }
        ...
        sqlx::query(
            ...
            INSERT INTO cost_records
                (id, api_key_id, provider, model, input_tokens, output_tokens,
                 cost_micro_usd, request_id, fallback_used)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            "#,
        )
        .bind(&record_id)
    ...
    async fn get_daily_spend(&self, key_id: &str) -> i64 {
        let key = Self::daily_key(key_id);
        let mut conn = self.redis.lock().await;
    ...
    async fn get_monthly_spend(&self, key_id: &str) -> i64 {
        let key = Self::monthly_key(key_id);
        let mut conn = self.redis.lock().await;
        conn.get::<_, Option<i64>>(&key)
            .await
            .unwrap_or(None)
    ...
    pub async fn get_spend_summary(
        ...
        let today = self.get_daily_spend(key_id).await;
        let month = self.get_monthly_spend(key_id).await;
